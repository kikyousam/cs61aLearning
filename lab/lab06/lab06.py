...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    change_rest = make_change(amount-smallest, rest)
    if not change_rest:
        return make_change(amount,rest)
    else:
        return [smallest] + change_rest
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        change_back = make_change(coin,self.coins)
        if not change_back:
            return [coin]
        else:
            for coin_changed in change_back:
                self.coins = remove_one(self.coins, coin_changed)
