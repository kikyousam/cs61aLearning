...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps_selected = [s for s in paragraphs if select(s)]
    if k <= len(ps_selected) - 1:
...
def about(subject):
    ...
    def f(text):
        ...
        return any([x in preprocessed_text for x in subject])
    return f
    # END PROBLEM 2
...
def furry_fixes(typed, source, limit):
    ...
    if len(source) == 0:
        if  len(typed) > limit:
            return limit + 1
        return len(typed)
    furry_fixes_number = furry_fixes(typed[1:], source[1:], limit)
    if typed[0] == source[0]:
        if  furry_fixes_number > limit:
            return limit + 1
        return furry_fixes_number
    else:
        if  furry_fixes_number + 1 > limit:
            return limit + 1
        return 1 + furry_fixes_number
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if abs(len(typed) - len(source)) > limit:
        return limit + 1
    if min(len(typed), len(source)) == 0: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
    ...
    substitute = diff + minimum_mewtations(typed[1:],source[1:],limit-diff)
    # BEGIN
    "*** YOUR CODE HERE ***"
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fw_list = [[] for _ in player_indices]
    for i in word_indices:
        fastest_player = 0
        min_use_time = 0xffff
        for j in player_indices:
            if get_time(times, j, i) < min_use_time:
                min_use_time = get_time(times, j, i)
                fastest_player = j
        fw_list[fastest_player].append(words[i])
    return fw_list
    # END PROBLEM 10
