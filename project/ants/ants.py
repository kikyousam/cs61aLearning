...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        distance = 0
        while place.entrance is not None:
            if not place.is_hive and distance >= self.lower_bound and distance <= self.upper_bound:
                if len(place.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0 and bees is not None:
            print("Debug ", len(bees))
            for bee in bees[:]:
                print("Debug", bee.health)
    ...
    def action(self, gamestate):
        if self.cooldown == 0:
            if self.place.bees is not None and len(self.place.bees) > 0:
                    ...
                    print("Debug", type(bee))
                    bee.reduce_health(bee.health)
                    self.cooldown = self.chew_cooldown
        else:
            self.cooldown -= 1
            print("Debug", self.cooldown)
# END Problem 7
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants also offense bees."""
    name = 'Tank'
    damage = 1
    food_cost = 6
    implemented = True
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place.exit != None:
            ...
            if place.ant is None:
                continue
            print("Debug", type(place.ant))
            if place.ant.is_container and place.ant.ant_contained != None:
